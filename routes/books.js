...
.get((req, res) => {
  ...
  .post((req, res) => {
    k('books').returning(['id', 'title', 'author', 'genre', 'description', 'cover_url'])
...
.get((req, res) => {
  ...
  .patch((req, res) => {
    k('books').where('id', req.params.id).returning(['id', 'title', 'author', 'genre', 'description', 'cover_url'])
      .update(h.decamelizeKeys(req.body)).then((book) => {
        res.send(h.camelizeKeys(book[0]));
      });
  })
  .delete((req, res) => {
    k('books').where('id', req.params.id).returning(['title', 'author', 'genre', 'description', 'cover_url'])
      .del().then((book) => {
        res.send(h.camelizeKeys(book[0]));
      });
  });
