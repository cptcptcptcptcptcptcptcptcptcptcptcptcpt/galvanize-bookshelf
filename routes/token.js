...
r.route('/').get((req, res, next) => {
  !req.cookies.token ? res.send(false) : res.send(true);
}).post((req, res, next) => {
  knex('users').select().then((usr) => {
    ...
    usrO.email === req.body.email && bcrypt(req.body.password, usr[0].hashed_password)?res.cookie('token', token, {
      httpOnly: true }).send(usrO) : next(bam);
  })
}).delete((req, res, next) => {
