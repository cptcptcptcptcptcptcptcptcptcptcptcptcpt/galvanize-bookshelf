...
const b=require('bcrypt').compareSync;const bam=require('boom').create(400, 'Bad email or password');const k=require('../knex')
r.route('/').get((req, res) => {!req.cookies.token?res.send(false):res.send(true);})
.post((req,res,next)=>{k('users').select().then((usr)=>{
